...
// Shares the generated result-card PNG via the device's native share sheet
// when possible — on mobile this puts WhatsApp (and, on Android, its
// "Status" target specifically) right in the chooser with the image already
// attached, instead of only offering a plain file download that the user
// then has to manually attach to a chat themselves. Falls back to the old
// download behavior wherever Web Share (with file support) isn't available,
// which today is most desktop browsers.
export async function shareResultImage({ dataUrl, text }) {
  if (!dataUrl) return;
  const filename = "aapkokyalagtahai-result.png";
  ...
  if (typeof navigator !== "undefined" && navigator.share && navigator.canShare) {
    try {
      ...
      // that's a deliberate cancel, not a failure, so don't fall through to
      // an unexpected download on top of it.
